...
// To be completed...
  ...
  for (int i = 1; i <=2; i++) { //i is less than/equal to 2, increment i
    ...
    if (!(tmpy > 7)) { //if piece is not off the board
      ...
      if (y == 1) { //if y co-ordinate is equal to 1
  ...
  //For pawn is not at startig position & wants to make a diagonal move bottom right
  for (int i = 1; i < 2; i++) {
    ...
    if (!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)) {
      ...
      if (piecePresent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) { //if a piece is present
        if (checkWhiteOponent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) { //if piece is an opponent piece
  ...
  for (int i = 1; i < 2; i++) {
    ...
    if (!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)) {
      ...
      if (piecePresent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) { //checking if a piece is present
        if (checkWhiteOponent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) { //checking if piece is an enemy piece
          moves.push(validM4);
        }
      }
    }
...
private Stack getKnightMoves(int x, int y, String piece){
  ...
  /*
      ...
      You just need to make sure that the L shape move stays on th =e board and doesn't
      take its own piece.
  */
...
private void printStack(Stack input){
  ...
  private void makeAIMove(){
    ...
    */
    ...
    Stack black = findBlackPieces(); //find all black pieces. Stack of where all black pieces start from
    Stack completeMoves = new Stack(); //instanciate a new empty stack for complete movements
    Move tmp;
    while(!white.empty()){ //while
